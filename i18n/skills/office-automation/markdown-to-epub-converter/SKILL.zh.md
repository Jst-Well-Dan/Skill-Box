...
1. **使用适当的 Markdown 结构**：当 markdown 遵循标准约定时（H1 用于标题，H2 用于部分标题），该技能效果最佳

2. **清晰的章节分隔**：使用 H1 标题清楚地标记章节分隔

3. **描述性标题**：标题成为目录，因此请确保它们清晰且具有描述性

4. **内容组织**：在标题之间逻辑地放置内容

5. **支持的格式**：坚持基本的 markdown 格式化以获得最佳兼容性

## 故障排除

**EPUB 无法打开**：请确保您的 markdown 格式正确。检查链接中的匹配括号和正确的语法。

**目录为空**：请确保您的 markdown 包含 H1 标题以定义章节。

**格式看起来不同**：电子书阅读器应用自己的字体和样式。这是正常和预期行为。
