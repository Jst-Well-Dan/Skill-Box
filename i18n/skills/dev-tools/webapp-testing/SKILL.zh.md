...
**始终先运行 `--help` 来查看用法**。 在尝试运行脚本并发现需要绝对必要的定制解决方案之前，不要阅读源代码。这些脚本可以非常大，因此会污染您的上下文窗口。它们存在是为了直接作为黑盒脚本调用，而不是被摄入到您的上下文窗口中。
...
with sync_playwright() as p:
    browser = p.chromium.launch(headless=True) # 总是在无头模式下启动铬
...
- **将捆绑脚本作为黑盒使用** - 为了完成一项任务，考虑是否可以使用 `scripts/` 中可用的脚本之一。这些脚本可以可靠地处理常见的复杂工作流程，而不会污染上下文窗口。使用 `--help` 查看用法，然后直接调用。
