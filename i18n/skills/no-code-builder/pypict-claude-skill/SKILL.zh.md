...
description: 使用 PICT (成对独立组合测试) 设计针对任何需求或代码的综合测试用例。分析输入，使用成对测试生成具有参数、值和约束的有效场景的 PICT 模型。输出 PICT 模型、测试用例的 Markdown 表格和预期结果。
---

# PICT 测试设计师

此技能通过使用 PICT (成对独立组合测试) 实现系统化的测试用例设计。给定需求或代码，它分析系统以识别测试参数，生成具有适当约束的 PICT 模型，执行模型以生成成对测试用例，并以预期输出格式化结果。
...
**参考 references/pict_syntax.md 以获取**：
- 完整的语法参考
- 约束语法和运算符
- 高级功能（子模型、别名、负测试）
- 命令行选项
...
**使用辅助脚本（可选）：**
`scripts/pict_helper.py` 脚本提供了模型生成和输出格式化的实用工具：
...
**用户请求**：“为具有支付方式、配送选项和用户类型的结账流程设计测试。”
...
**输出**：12-15 个测试用例，涵盖所有有效的支付/配送/用户组合以及预期的成本和结果。