...
### 零公式错误
- 每个Excel模型必须交付时带有零公式错误（#REF!、#DIV/0!、#VALUE!、#N/A、#NAME?）
...
#### 硬编码的文档要求
- 注释或单元格旁边的单元格（如果表格末尾）。格式：“来源：[系统/文档]，[日期]，[具体参考]，[如有适用，请提供URL]”
- 示例：
...
**LibreOffice必需用于公式重新计算**：您可以使用`recalc.py`脚本来假设安装了LibreOffice，以重新计算公式值。脚本在首次运行时自动配置LibreOffice
...
## CRITICAL：使用公式，而不是硬编码的值

**始终使用Excel公式而不是在Python中计算值并硬编码它们。** 这确保了电子表格保持动态和可更新的。

### ❌ 错误 - 硬编码计算值
```python
# Bad：在Python中计算并在Python中硬编码结果
total = df['Sales'].sum()
...
3. **修改**：添加/编辑数据、公式和格式
4. **保存**：写入文件
5. **重新计算公式（如果使用公式则强制要求）**：使用recalc.py脚本
   ```bash
...
由openpyxl创建或修改的Excel文件包含作为字符串的公式而不是计算值。使用提供的`recalc.py`脚本来重新计算公式：
...
- [ ] **远端列**：财年数据通常在列50+
- [ ] **多个匹配**：搜索所有发生，而不仅仅是第一个
- [ ] **除以零**：在公式中使用`/`之前检查除数（#DIV/0!）
- [ ] **错误的引用**：验证所有单元格引用是否指向目标单元格（#REF!）
- [ ] **跨工作表引用**：使用正确的格式（Sheet1!A1）进行链接

### 公式测试策略
- [ ] **从小处开始**：在广泛应用之前在2-3个单元格上测试公式
...
### 库选择
- **pandas**：最佳用于数据分析、批量操作和简单的数据导出
- **openpyxl**：最佳用于复杂的格式、公式和Excel特定功能

### 使用openpyxl
- 单元格索引是1基于的（行=1，列=1表示单元格A1）
