...
### 🎥 Content Harvesting & Creation
- **`jina-cli` & `content-harvester`** — The "eyes" for web content structuring and the triple-threat for video/WeChat scraping.
- **`baoyu-content-creation-suite`** — All-in-one suite for Xiaohongshu/WeChat visuals and professional infographics.

### 📊 Business Analysis & Automation
- **`invoice-processor`** — Automated PDF/image invoice extraction to Excel via AI vision.
- **`document-suite` & `md-to-pdf`** — Official Anthropic toolkit and CJK-supported PDF conversion for the final mile of content output.
- **`notebooklm-integration`** — Deep document analysis powered by Google NotebookLM with source-based citations.

### 📝 Knowledge Management
- **`obsidian-toolkit`** — The authoritative toolkit from Obsidian founder kepano, covering Markdown mastery and CLI automation.

### 💻 Dev & Meta
- **`anthropic-frontend-design` & `vercel-frontend-suite`** — Official tools for high-quality UI design and instant Vercel deployment.
