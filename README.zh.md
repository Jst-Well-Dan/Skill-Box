...
<p align="center">
  <img src="https://img.shields.io/badge/技能-78-orange?style=for-the-badge" alt="Skills">
...
| 分类 | 说明 | 技能数 |
| :--- | :--- | :---: |
| **开发工具 (Dev Tools)** | 前端开发、测试、DevOps 及 AI 元技能 | 11 |
| **内容流水线 (Content Pipeline)** | 网页采集、视频下载、AI 内容创作及发布 | 15 |
...
> *“不再是零散的工具，而是经过高频实战验证的专业闭环。”*

### 📊 投行与商业分析
> *结合 CFA 专业背景实战验证，将耗时的金融建模和财务数据提取自动化。*
- **`alpha-vantage` & `edgartools`** — 直接访问美股 SEC 报表 (10-K/10-Q) 与实时基本面。
- **`excel-dcf-modeler` & `excel-lbo-modeler`** — 自动生成专业的估值模型。
- **`invoice-processor`** — 通过 AI 视觉将 PDF/图片发票提取至 Excel。

### 📝 深度学习与第二大脑
> *个人知识管理 (PKM) 的核心链路，解决“读完就忘”的痛点。*
- **`obsidian-toolkit`** — Obsidian 终极工作流：CLI、Markdown 指南及自动化的 Bases 视图。
- **`excalidraw-diagram`** — 将文字逻辑一键转化为适配 Obsidian 的 Excalidraw 架构图。

### 🎥 深度内容采编
> *解决复杂网页与封闭平台的内容抓取难题。*
- **`weixin-fetch`** — 经过优化的微信公众号文章抓取（含破防爬）。
- **`advanced-video-downloader`** — 极速下载 YouTube、B站等 1000+ 平台视频。
- **`youtube-transcript`** — 提取精准字幕，构建你的知识库。
...
<details>
<summary>点击展开 7 大分类 / 78 个技能完整列表</summary>

### 开发工具 Dev Tools（11 个技能）
...
| 2 | `canvas-design` | 精美静态视觉艺术与海报创作 | [Anthropic](https://github.com/anthropics/skills) |
