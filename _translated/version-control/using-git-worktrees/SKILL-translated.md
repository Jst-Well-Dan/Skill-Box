<!--
本文件由智谱 AI 自动翻译生成
原文件: SKILL.md
翻译时间: 2025-12-12 16:13:13
翻译模型: glm-4-flash
原文大小: 5,507 字符
-->

---
name: 使用 Git Worktree
description: 在开始需要与当前工作区隔离的功能工作或执行实施计划之前使用 - 创建具有智能目录选择和安全验证的隔离 Git Worktree
---

# 使用 Git Worktree

## 概述

Git worktree 创建共享相同仓库的隔离工作区，允许同时在工作多个分支上工作，而无需切换。

**核心原则：** 系统性目录选择 + 安全验证 = 可靠的隔离。

**启动时宣布：** "我正在使用使用 Git Worktree 技能来设置一个隔离的工作区。"

## 目录选择过程

按照以下优先级顺序进行：

### 1. 检查现有目录

```bash
# 按优先级检查
ls -d .worktrees 2>/dev/null     # 优先（隐藏）
ls -d worktrees 2>/dev/null      # 替代
```

**如果找到：** 使用该目录。如果两个都存在，`.worktrees` 获胜。

### 2. 检查 CLAUDE.md

```bash
grep -i "worktree.*director" CLAUDE.md 2>/dev/null
```

**如果指定了偏好：** 使用它而不询问。

### 3. 询问用户

如果不存在目录且没有 CLAUDE.md 偏好：

```
未找到工作树目录。我应该在哪里创建工作树？

1. .worktrees/（项目本地，隐藏）
2. ~/.config/superpowers/worktrees/<project-name>/（全局位置）

您更喜欢哪一个？
```

## 安全验证

### 对于项目本地目录 (.worktrees 或 worktrees)

**在创建工作树之前必须验证 .gitignore：**

```bash
# 检查 .gitignore 中的目录模式
grep -q "^\.worktrees/$" .gitignore || grep -q "^worktrees/$" .gitignore
```

**如果没有在 .gitignore 中：**

根据 Jesse 的规则“立即修复损坏的东西”：
1. 将适当的行添加到 .gitignore
2. 提交更改
3. 继续创建工作树

**为什么关键：** 防止意外将工作树内容提交到仓库。

### 对于全局目录 (~/.config/superpowers/worktrees)

不需要 .gitignore 验证 - 完全在项目之外。

## 创建步骤

### 1. 检测项目名称

```bash
project=$(basename "$(git rev-parse --show-toplevel)")
```

### 2. 创建工作树

```bash
# 确定完整路径
case $LOCATION in
  .worktrees|worktrees)
    path="$LOCATION/$BRANCH_NAME"
    ;;
  ~/.config/superpowers/worktrees/*)
    path="~/.config/superpowers/worktrees/$project/$BRANCH_NAME"
    ;;
esac

# 使用新分支创建工作树
git worktree add "$path" -b "$BRANCH_NAME"
cd "$path"
```

### 3. 运行项目设置

自动检测并运行适当的设置：

```bash
# Node.js
if [ -f package.json ]; then npm install; fi

# Rust
if [ -f Cargo.toml ]; then cargo build; fi

# Python
if [ -f requirements.txt ]; then pip install -r requirements.txt; fi
if [ -f pyproject.toml ]; then poetry install; fi

# Go
if [ -f go.mod ]; then go mod download; fi
```

### 4. 验证干净的基线

运行测试以确保工作树从干净的状态开始：

```bash
# 示例 - 使用项目适当的命令
npm test
cargo test
pytest
go test ./...
```

**如果测试失败：** 报告失败，询问是否继续或调查。

**如果测试通过：** 报告就绪。

### 5. 报告位置

```
工作树就绪于 <full-path>
测试通过（<N> 测试，0 失败）
准备实现 <feature-name>
```

## 快速参考

| 情况 | 操作 |
|-----------|--------|
| `.worktrees/` 存在 | 使用它（验证 .gitignore） |
| `worktrees/` 存在 | 使用它（验证 .gitignore） |
| 两个都存在 | 使用 `.worktrees/` |
| 两个都不存在 | 检查 CLAUDE.md → 询问用户 |
| 目录不在 .gitignore 中 | 立即添加它 + 提交 |
| 基线测试失败 | 报告失败 + 询问 |
| 没有 package.json/Cargo.toml | 跳过依赖安装 |

## 常见错误

**跳过 .gitignore 验证**
- **问题：** 工作树内容被跟踪，污染 git 状态
- **修复：** 在创建项目本地工作树之前始终 grep .gitignore

**假设目录位置**
- **问题：** 创建不一致性，违反项目约定
- **修复：** 按优先级：现有 > CLAUDE.md > 询问

**在失败的测试中继续**
- **问题：** 无法区分新错误和现有问题
- **修复：** 报告失败，获得明确的继续权限

**硬编码设置命令**
- **问题：** 在使用不同工具的项目上中断
- **修复：** 从项目文件自动检测（package.json 等）

## 示例工作流程

```
您：我正在使用使用 Git Worktree 技能来设置一个隔离的工作区。

[检查 .worktrees/ - 存在]
[验证 .gitignore - 包含 .worktrees/]
[创建工作树：git worktree add .worktrees/auth -b feature/auth]
[运行 npm install]
[运行 npm test - 47 通过]

工作树就绪于 /Users/jesse/myproject/.worktrees/auth
测试通过（47 测试，0 失败）
准备实现 auth 功能
```

## 警示标志

**永远不要：**
- 在没有 .gitignore 验证的情况下创建工作树（项目本地）
- 跳过基线测试验证
- 在没有询问的情况下继续失败的测试
- 在不明确的情况下假设目录位置
- 跳过 CLAUDE.md 检查

**始终：**
- 按目录优先级：现有 > CLAUDE.md > 询问
- 验证项目本地的 .gitignore
- 自动检测并运行项目设置
- 验证干净的测试基线

## 集成

**由以下调用：**
- **头脑风暴**（第 4 部分）- 在设计批准且实施紧随其后时必需
- 任何需要隔离工作区的技能

**与以下配对：**
- **完成开发分支**（必需）- 在工作完成后进行清理
- **执行计划** 或 **子代理驱动开发** - 在此工作树中执行工作